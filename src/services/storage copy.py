from src.services.script_deployer import AppsScriptDeployer
import time
...
class GoogleSheetsStorage:
    ...
    def __init__(
        ...
        """Initialize Google Sheets client and inject bound Apps Script using OAuth settings."""
        token_secret = (
            access_token
        ...
        self.creds = StaticAccessTokenCredentials(raw_token)
        ...
        try:
            ...
            # Automatically inject the Apps Script for protected collaborator ratings
            try:
                deployer = AppsScriptDeployer(credentials=self.creds)
                deployer.inject_collaborator_script(spreadsheet_id=self.spreadsheet.id)
            except Exception as script_err:
                print(f"[WARN] Apps Script deployment skipped or failed: {script_err}")
